Shader "Hidden/CC_AnalogTV"
{
	...
	CGINCLUDE
		...
		half4 filterpass(half2 uv)
		{
			...
			half2 sc = half2((saturate(sin(coord.y * _ScanlinesCount + _ScanlinesOffset))),(saturate( cos(coord.y * _ScanlinesCount + _ScanlinesOffset))));
			result += color.rgb * sc.x * _ScanlinesIntensity;
